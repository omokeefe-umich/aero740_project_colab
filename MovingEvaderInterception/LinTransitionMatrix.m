...
function [S, M] = LinTransitionMatrix(dt_by_level, timestep_levels, Ac, Bc)
    ...
    M(1:N_x, :) = squeeze(Ads(timestep_levels(1), :, :));
    S(1:N_x, 1:N_u) = squeeze(Bds(timestep_levels(1), :, :));
    
    for i = 1 : N_steps - 1
        M(N_x * i + 1:N_x * (i + 1), :) = squeeze(Ads(timestep_levels(i + 1), :, :)) * M(N_x * (i - 1) + 1:N_x * i, :);
        S(N_x * i + 1:N_x * (i + 1), :) = squeeze(Ads(timestep_levels(i + 1), :, :)) * S(N_x * (i - 1) + 1:N_x * i, :);
        S(N_x * i + 1:N_x * (i + 1), N_u * i + 1:N_u * (i + 1)) = squeeze(Bds(timestep_levels(i + 1), :, :));
    end
end
